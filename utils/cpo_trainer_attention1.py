...
class CPOTrainer(Trainer):
    ...
    def cpo_loss(
        ...
        def attention_similarity(chosen_hidden_states: torch.Tensor,
                         rejected_hidden_states: torch.Tensor) -> torch.Tensor:
            """
            Compute similarity between chosen and rejected using scaled dot-product attention.
            Uses attention output = softmax(QK^T / sqrt(d_k)) @ V,
            then takes the max over embedding per token, followed by mean over tokens.

            Args:
                chosen_hidden_states: Tensor of shape (B, S, E)
                rejected_hidden_states: Tensor of shape (B, S, E)

            Returns:
                similarity: Tensor of shape (B,)
            """
            Q = chosen_hidden_states                           # (B, S, E)
            K = rejected_hidden_states                         # (B, S, E)
            V = rejected_hidden_states                         # (B, S, E)

            d_k = Q.size(-1)
            scale = d_k ** 0.5

            # Scaled dot-product attention weights
            attn_scores = torch.bmm(Q, K.transpose(-1, -2)) / scale   # (B, S, S)
            attn_weights = F.softmax(attn_scores, dim=-1)             # (B, S, S)

            # Attention output
            attn_output = torch.bmm(attn_weights, V)                  # (B, S, E)

            # Max over embedding dim (E) per token, then mean over tokens (S)
            max_per_token = attn_output.max(dim=-1).values            # (B, S)
            similarity = max_per_token.mean(dim=-1)                   # (B,)

            return similarity
                
            #penalty_cos = scale_cosine_similarity_torch(word_level_cosine_similarity(policy_chosen_logits, policy_rejected_logits))
        penalty_atten = attention_similarity(policy_chosen_logits, policy_rejected_logits)
