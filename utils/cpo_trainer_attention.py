    ...
    def cpo_loss(
        ...
        class AttentionSimilarity(nn.Module):
            ...
            def forward(self, chosen_hidden_states, rejected_hidden_states):
                """
                Computes attention-based similarity between two sequences.

                Args:
                    chosen_hidden_states: Tensor of shape (B, S, D)
                    rejected_hidden_states: Tensor of shape (B, S, D)

                Returns:
                    similarity: Tensor of shape (B,) — similarity score for each example
                """
                Q = self.W_Q(chosen_hidden_states) 
                K = self.W_K(rejected_hidden_states)

                d_k = Q.size(-1)
                scores = torch.bmm(Q, K.transpose(1, 2)) / math.sqrt(d_k)  # (B, S, S)

                attn_weights = F.softmax(scores, dim=-1)  # (B, S, S)
                similarity = attn_weights.mean(dim=-1).max(dim=-1).values  # (B,)
