...
class CPOTrainer(Trainer):
    ...
    def cpo_loss(
        ...
        def sigmoid_dot_similarity(logits1, logits2):
            """
            Computes sigmoid(dot product) similarity between 2D tensors.

            Args:
                logits1, logits2: Tensors of shape [batch_size, feature_dim]

            Returns:
                Tensor of shape [batch_size] with similarity scores.
            """
            dot_product = torch.sum(logits1 * logits2, dim=1)   # [batch_size]
            similarity = torch.sigmoid(dot_product)             # [batch_size]
            return similarity
            #penalty_cos = scale_cosine_similarity_torch(word_level_cosine_similarity(policy_chosen_logits, policy_rejected_logits))
        penalty_ndp = sigmoid_dot_similarity(policy_chosen_logits, policy_rejected_logits)

        print('penalty_ndp:', penalty_ndp)
        logits = (policy_chosen_logps - (1- penalty_ndp).clamp(0.0, 1.0)* policy_rejected_logps).to(self.accelerator.device)
